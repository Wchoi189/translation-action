...
for (const line of lines) {
  if (line.includes('OR_DEEPSEEK_R1T1')) {
    // Try to extract the value
    const match = line.match(/OR_DEEPSEEK_R1T1\s*=\s*["']?([^"'\s\n]+)["']?/);
    if (match) {
      apiKey = match[1].trim();
      break;
    }
...
if (!apiKey) {
  console.error('Could not find OR_DEEPSEEK_R1T1 in .env.local');
  process.exit(1);
}
...
Translation:`;
  ...
  try {
    ...
    console.log('📝 Using model: tngtech/deepseek-r1t2-chimera:free');
